...
export const generateBuildFiles = async (...args: any) => {
  ...
  await Promise.all([
    ...
    // Remove unused tslint configuration
    () => {
      const tslintPath = makePath(dir, Files.tslint)
      if (fs.existsSync(tslintPath)) {
        fs.unlink(makePath(dir, Files.tslint))
      }
    },
  ])

  // Run Migrations on code base source code
  runMigrationsOnTabCodebase(dir, miskTab.slug)

  // Append non-package JSON files with generated by comment
  await fs.writeFile(makePath(dir, Files.tsconfig), generatedByCLI, {
    flag: "a",
  })
  logDebug(
    tag,
